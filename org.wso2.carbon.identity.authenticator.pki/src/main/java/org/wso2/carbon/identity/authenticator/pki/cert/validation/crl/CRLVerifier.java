 ...
 * under the License.
 */
package org.wso2.carbon.identity.authenticator.pki.cert.validation.crl;
...
/**
 * This is used to verify a certificate is revoked or not by using the
 * Certificate Revocation List published
 * by the CA.
 */
public class CRLVerifier implements RevocationVerifier {
	 ...
	 * IssuerCertificate can be used
	 * to check if the CRL URL has the Issuers Domain name. But this is not
	...
	public RevocationStatus checkRevocationStatus(X509Certificate peerCert,
		...
		// check with distributions points in the list one by one. if one fails
		// go to the other.
		for (String crlUrl : list) {
			...
			// todo: Do we need to check if URL has the same domain name as
			// issuerCert?
		...
		throw new CertificateVerificationException(
		                                           "Cannot check revocation status with the certificate");
	}
	 ...
	 */
	protected X509CRL downloadCRLFromWeb(String crlURL) throws IOException,
		...
		try {
			...
			throw new CertificateVerificationException(
			                                           "Cannot generate X509CRL from the stream data",
			                                           e);
		} finally {
			if (crlStream != null)
				crlStream.close();
	...
	private List<String> getCrlDistributionPoints(X509Certificate cert)
		...
		byte[] crlDPExtensionValue =
		                             cert.getExtensionValue(X509Extensions.CRLDistributionPoints.getId());
		if (crlDPExtensionValue == null)
			throw new CertificateVerificationException(
			                                           "Certificate doesn't have CRL Distribution points");
